...
exports.chainWebpack = (webpackConfig) => {
  ...
  if (isClient) {
    ...
    if (modernMode) {
      ...
      if (!modernBuild) {
        webpackConfig.plugin('loader').use(WebpackBar, [{ name: 'Client (legacy)', color: '#2ECC71' }])
        ...
        webpackConfig.plugin('loader').use(WebpackBar, [{ name: 'Client (modern)', color: '#F8F409' }])
    ...
    webpackConfig.plugin('loader').use(WebpackBar, [{ name: 'Server', color: '#09D4F8' }])
